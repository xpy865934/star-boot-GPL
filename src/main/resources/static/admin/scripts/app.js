        ...
        $httpProvider.interceptors.push(function ($q) {
            return {
                'request': function (config) {
                    ...
                    if (config.url && config.url.indexOf('/app') === 0 || config.url && config.url.indexOf('/starflowable') === 0) {
                        // 修改请求访问路径
                        config.url = FlowableAdmin.contextRoot + config.url;
    ...
    .filter('dateformat', function() {
        ...
        .run(['$rootScope', '$http', '$timeout', '$location', '$cookies', '$modal', '$translate', '$window',
            function($rootScope, $http, $timeout, $location, $cookies, $modal, $translate, $window) {
        		...
        		$http.get('/starflowable/rest/account')
